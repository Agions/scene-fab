...
规则:
 ...
 if section.section_type == MusicSection.CHORUS:
 clip_idx = self._hybrid_chorus_section(
 plan, section_beats, num_clips, min_dur, clip_idx
 )
 elif section.section_type in (MusicSection.INTRO, MusicSection.OUTRO):
 clip_idx = self._hybrid_boundary_section(
 plan, section, num_clips, clip_idx
 )
 else:
 clip_idx = self._hybrid_narrative_section(
 plan, section_beats, num_clips, min_dur, clip_idx
 )

 def _hybrid_chorus_section(
 self,
 plan: SyncPlan,
 section_beats: list,
 num_clips: int,
 ...
 def _hybrid_boundary_section(
 self,
 plan: SyncPlan,
 section: "MusicSection", # type:ignore[name-defined]
 num_clips: int,
 clip_idx: int,
 ) -> int:
 """Intro/Outro段:整个段落一个片段（慢节奏）"""
 ...
 """Verse/Bridge段: 每4 个节拍切一次（叙事感）"""
 # Verse/Bridge: 每4 个节拍切一次
 ...
 avg_energy = sum(energies) / len(energies) if energies else0.5
