...
class SyncEngine:
    ...
    def _hybrid_sync(
        ...
        for section in sections:
            ...
            clip_idx = self._apply_hybrid_section(
                plan, section, section_beats, clip_idx, num_clips, min_dur
            )

    def _apply_hybrid_section(
        self,
        plan: SyncPlan,
        section,
        section_beats: list,
        ...
        """对单个段落应用混合策略 — 返回更新后的 clip_idx"""
        section_type = section.section_type

        if section_type == MusicSection.CHORUS:
            return self._hybrid_chorus_beats(
                plan, section_beats, clip_idx, num_clips, min_dur
            )
        elif section_type in (MusicSection.INTRO, MusicSection.OUTRO):
            return self._hybrid_intro_outro(
                plan, section, clip_idx, num_clips
            )
        else:
            # Verse/Bridge：每 4 个节拍切一次
            return self._hybrid_verse_beats(
                plan, section_beats, clip_idx, num_clips, min_dur
            )

    def _hybrid_chorus_beats(
        self,
        plan: SyncPlan,
        section_beats: list,
        clip_idx: int,
    ...
    @staticmethod
    def _is_too_close(plan: SyncPlan, timestamp: float, min_dur: float) -> bool:
        """检查与上一个切点的间隔 — 提取重复的 min_dur 检查"""
        if not plan.sync_points:
            return False
        return timestamp - plan.sync_points[-1].timestamp < min_dur
