...
```bash
# 使用 ghproxy 代理 (替换 <version> 为最新版本号, 如 v2.4.0)
wget https://ghproxy.com/https://github.com/Agions/scene-fab/releases/download/<version>/SceneFab-x.x.x.AppImage
```
...
| Kimi | ✅ 支持 | 需配置 API Key |
| GLM-5 | ✅ 支持 | 需配置 API Key |
| 豆包 | ✅ 支持 | 需配置 API Key |
| 混元 | ✅ 支持 | 需配置 API Key |
| 本地模型 | ✅ 支持 | 通过 Ollama / LM Studio |
