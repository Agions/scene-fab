...
能。Linux 服务器或 Docker 容器中，应用会自动使用 `QT_QPA_PLATFORM=offscreen` 模式，完整运行所有 AI 处理流程。
...
可能是触发了 API 的速率限制（Rate Limit）。等待 1 分钟重试。

### 支持其他 AI 提供商吗？

目前 SceneFab 主推 DeepSeek-V4 + Qwen VL（性价比最优）。理论上支持 OpenAI GPT-4，但需要自行修改代码中的 API 端点。
...
能，但建议分段处理（每段 10–15 分钟效果最佳）。超长视频会自动分段处理。
...
能。SceneFab 在无 GPU 时自动回退到 CPU 模式。视频理解会变慢，其他步骤几乎不受影响。
...
- **GitHub Releases**：关注 [Releases 页面](https://github.com/Agions/scene-fab/releases)
