...
SceneFab 的架构目标是让短剧/影视解说生产流程**可重复、可验证、可扩展**。系统不依赖手动剪辑时间线，而是围绕"素材理解 → 剧情上下文 → 第一人称脚本 → 配音字幕 → 导出发布"建立稳定的数据流管线。
...
| AI 服务 | `services/ai` | LLM 适配、视觉理解、ASR 语音识别、TTS 配音合成 |
...
1. **结构化优先** — 生产字段使用结构化数据模型，避免依赖文件名和自由文本。
2. **上下文共享** — 脚本生成和质量评估使用同一套 NarrationContext，确保一致性。
3. **优雅降级** — AI 服务失败时可降级到备选模型或跳过非关键步骤，不阻断基础流程。
4. **导出标准化** — 导出预设和文档标准保持一致，支持多平台一键输出。
5. **UI 职责单一** — 桌面工作台只承载生产决策和状态反馈，不复制业务规则。
6. **模型单源真相** — 模型名以 `services.ai.model_catalog` 为唯一数据源，配置文件、Provider 默认值和设置页不得各自硬编码。
