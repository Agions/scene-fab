...
SceneFab 在设计之初就将安全作为核心考量，保护用户的敏感数据和隐私。
...
from scenefab.core.secure_file_handler import SecureFileHandler
...
```python
# 示例：安全执行 FFmpeg
from scenefab.core.command_validator import CommandValidator
...
```bash
# 插件开发者签名
python -m scenefab plugins sign ./plugins/my-plugin

# SceneFab 验证签名
python -m scenefab plugins install ./my-plugin-1.0.0.sfplugin
# → 签名验证通过后才安装
```
...
```bash
# 清除所有本地缓存
python -m scenefab cache clear

# 清除项目数据（不可恢复）
python -m scenefab data purge --project=my-project
```
...
| 方式 | 说明 |
|------|------|
| GitHub Security Advisory | [报告安全漏洞](https://github.com/Agions/scene-fab/security/advisories/new) |
| 邮件 | security@scenefab.ai |
...
| 配置项 | 文件位置 | 说明 |
|--------|----------|------|
| API Key 存储 | `~/.scenefab/credentials.enc` | 加密文件 |
| 信任密钥 | `~/.scenefab/trusted_keys/` | 插件签名公钥 |
| 插件目录 | `./plugins/` | 插件加载目录 |
| 日志 | `~/.scenefab/logs/` | 操作日志（不含敏感数据） |
| 缓存 | `~/.scenefab/cache/` | 临时缓存 |
